...
B = 200 #最大随机数
COUNT = 300
randomSeed = 15 #random seed depended on the weight (KEY)
...
def suijishu():
    tWeight.delete(1.0, 'end')
    tWeight2.delete(1.0, 'end')
    strRead = ser.readline()
    tWeight.insert('end', strRead.decode())
    tWeight2.insert('end', strRead.decode())
    tWeight.after(100, suijishu)  
...
tWeight = tkr.Text(player,width = 8, height = 1)
...
tWeight2 = tkr.Text(player,width = 8, height = 1)
